...
@Component({
  selector: 'app-article-replies-list',
  templateUrl: './article-replies-list.component.html',
  styleUrls: ['./article-replies-list.component.scss'],
...
export class ArticleRepliesListComponent implements OnInit {
  userInfo: any;
  communities: any[] = [];
  community:any;
  commentId:number;
  ...
  addComment() {
    ...
    this.httpService.create(url, obj).subscribe(
      (res: any) => {
        // if (!(this.commentId)) {
        //   this.community.articclesLikeCommentCount.comment_count = this.community.articclesLikeCommentCount.comment_count + 1;
        // }
