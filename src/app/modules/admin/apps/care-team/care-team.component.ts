...
export class CareTeamComponent implements OnInit {
  ...
  openAddUser() {
    this.dialog
      .open(AddUserModalComponent, {
        ...
        height: '100%',
        position: { right: '0' },
        data: { roleId: 4, type: 'Care Coordinator' }
      })
      .afterClosed()
      .subscribe((data) => {
        if (data) {
  ...
  getAllRecords() {
    ...
    this.httpService.create(url, body).subscribe((res: any) => {
      ...
      if (usersData.length > 0) {
        let careTeamInfo = "Care Coordinator Info";
  ...
  SaveActivity(firstName: string,lastName: string, statusId: any) {
    ...
    const body = {
      ...
      descriptionname: statusId === 0 ? "Removed <b>"+ name +" - Care Coordinator" : statusId === 27 ? "Activated <b>"+ name +" - Care Coordinator </b>" : "Deactivated <b> "+ name +" - Care Coordinator </b>",
