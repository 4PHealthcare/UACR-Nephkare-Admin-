...
export class CustomerSupportComponent implements OnInit {
  ...
  openAddUser() {
    this.dialog
      .open(AddUserModalComponent, {
        ...
        height: '100%',
        position: { right: '0' },
        data: { roleId: 6, type: 'Customer Support' },
      })
      .afterClosed()
      .subscribe((data) => {
        if (data) {
  ...
  getAllRecords() {
    ...
    this.httpService.create(url, body).subscribe((res: any) => {
      ...
      if (usersData.length > 0) {
            let customerSupportInfo = "Customer Support Info";
  ...
  SaveActivity(firstName: string,lastName: string, statusId: any) {
    ...
    const body = {
      ...
      descriptionname: statusId === 0 ? "Removed <b>"+ name +" - Customer Support" : statusId === 27 ? "Activated <b>"+ name +" - Customer Support </b>" : "Deactivated <b> "+ name +" - Telecaller </b>",
