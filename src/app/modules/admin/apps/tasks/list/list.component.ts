...
export class TasksListComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        this._activatedRoute.queryParams.subscribe(params => {
           this.serviceId = params['id'] === undefined ? 0 : parseInt(params['id']);
          });
        this.GetServices();
        this.offerGroups$
    ...
    GetPromoCodeGroups() {
       const url = `api/PromoCode/GetOffersList_bysubscription?SubscriptionId=${this.serviceId}`;
    ...
    offerEdit(offer: any, groupId: string) {
        this.dialog.open(OfferComponent, {
          ...
          height: "100%",
          position: { right: "0" },
          data: { offer, groupId, subscriptions:this.subscriptions },
        }).afterClosed().subscribe((data) => {
            if(data) {
                this.serviceId = 0;
                this.GetServices();
    ...
    GetServices() {
        const url = `api/Subscription/GetSubscriptionList?subscriptionTypeId=0`;
        this.httpService.getAll(url).subscribe((res: any) => {
            if(res?.isSuccess && res.data) {
