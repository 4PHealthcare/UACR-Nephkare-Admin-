...
export class OfferComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.taskForm = this._formBuilder.group({
      ...
      description    : [''],
      offerValue: new FormControl(null, [Validators.required, Validators.maxLength(2), Validators.pattern(/^-?(0|[1-9]\d*)?$/)]),
      validfrom  : new FormControl(null, [Validators.required]),
      validto : new FormControl(null, [Validators.required]),
      promoGroup_id : new FormControl(0, [])
  }); 

  this.subscriptions = this.data.subscriptions;
  if (this.data && this.data.offer) {
    this.selectedSubscription = [this.data.offer.subscriptionid];
    this.taskForm.patchValue({
      promocodeid: this.data.offer.promocodeid,
      promocode: this.data.offer.promocode,
      offerValue: this.data.offer.value,
      description: this.data.offer.description,
      validfrom: this.data.offer.validfrom != null ? moment(this.data.offer.validfrom).format('YYYY-MM-DD') : null ,
      validto: this.data.offer.validto != null ? moment(this.data.offer.validto).format('YYYY-MM-DD') : null ,
      promoGroup_id: this.data.groupId,
    });
  }
  ...
  const toDate = this.taskForm.controls.validto.value == null ? null : moment(this.taskForm.controls.validto.value).format('YYYY-MM-DD');
    if(this.editMode) {
        return  {
          ...
          promo_groupid: this.taskForm.controls.promoGroup_id.value ? this.taskForm.controls.promoGroup_id.value : 0,
          created_by: this.userInfo.user_id,
          value : this.taskForm.controls.offerValue.value ? parseInt(this.taskForm.controls.offerValue.value): 0
      }
    } else {
        return  {
          promocodeid: 0,
...
submit() {
  ...
  resetForm(): void {
    this.taskForm.reset();
    this.clearDate('validfrom');
    this.clearDate('validto');
