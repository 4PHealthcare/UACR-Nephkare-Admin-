    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                // Sign in successful
                if ( request.body.email === 'drramakrishna@hellokidney.ai' && request.body.password === 'admin' )
                {
                    return [
                        200,
                        {
                            user       : cloneDeep(this._user),
                            adminAccessToken: this._generateJWTToken(),
                            tokenType  : 'bearer'
                        }
                    ];
                }
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                const adminAccessToken = request.body.adminAccessToken;

                // Verify the token
                if ( this._verifyJWTToken(adminAccessToken) )
                {
                    return [
                        200,
                        {
